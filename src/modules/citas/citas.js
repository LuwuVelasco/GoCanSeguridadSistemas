document.addEventListener('DOMContentLoaded', function() {
  ...
  if (especialidadSelect && doctorSelect && reservarButton) {
      ...
      console.error('No se pudieron encontrar los elementos necesarios en el DOM');
  }
});

async function actualizarEspecialidades() {
  const especialidadSelect = document.getElementById('especialidad');
  especialidadSelect.innerHTML = '<option value="">Seleccionar especialidad...</option>'; // Reset especialidad select

  try {
      const response = await fetch('http://localhost/GoCan/src/modules/php/citas.php');
